...
void Texture::LoadTexture() {
	...
	// Parametri: indica el funcionamiento de la textura
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_REPEAT);// eje S paralelo a X, repetir sobre el eje para cubrir superficie
...
*/
	...
	*/
	...
	// filtros, distancia de visualización
	// linear: ubica 4 texels de color cercanos, promedia y aplica a las coordenadas
	// nearest: texel más cerca
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR);
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
//	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_NEAREST);
	//if(RGBA) {
	glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, width, height, 0, GL_RGBA, GL_UNSIGNED_BYTE, texData); // genera textura, pos 5 siempre a 0
	glGenerateMipmap(GL_TEXTURE_2D); // crea texturas a distancia, cerca, en el punto y lejos
	/*}else{
